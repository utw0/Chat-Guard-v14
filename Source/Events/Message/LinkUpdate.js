...
module.exports = {
  ...
  execute: async (client, msg) => {
    ...
    if (!data || !data.logKanali || !data.linkKoruma|| msg.author?.bot) return;
    ...
    if (data.guvenli.includes(user.id)) return;
      ...
      const e = new EmbedBuilder().setDescription(
        `Bu sunucuda bağlantılara izin verilmiyor, ${user}.`
      );
      const linkRegex = /(http[s]?:\/\/)(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_+.~#?&/=]*)/gi;  
      ...
      if (links && links.length > 0) {
        ...
        const logMsg = await logChannel.send({
          embeds: [
            new EmbedBuilder()
              .setDescription(
                `<@${member.id}> Link gönderdiği için uyarıldı\n\`\`\`${msg.content}\`\`\``
              )
          ...
          switch (m.customId) {
            ...
            case "linkkick": {
              ...
              const kickEmbed = new EmbedBuilder()
                .setTitle("Kicked")
                .setDescription(
                  `Link gönderdiğiniz için \`${msg.guild.name}\` sunucusundan atıldınız.`
                )
                .setTimestamp();
              ...
              member.send({
                ...
                member.kick({ reason: "Link Gönderileri Attığı İçin Kicklendi" });
              });
              break;
            }
          }
