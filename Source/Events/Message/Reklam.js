  ...
  execute: async (client, msg) => {
    ...
    if (data.guvenli.includes(user.id)) return;
      ...
      const e = new EmbedBuilder().setDescription(
        `Bu sunucuda Reklam Bağlantılarına izin verilmiyor, ${user}.`
      );
      const linkRegex = /(https:\/\/)?(www\.)?(discord\.gg|discord\.me|discordapp\.com\/invite|discord\.com\/invite)\/([a-z0-9-.]+)?/i; 
      ...
      if (links && links.length > 0) {
        ...
        const logMsg = await logChannel.send({
          embeds: [
            new EmbedBuilder()
              .setDescription(
                `<@${member.id}> Reklam bağlantısı gönderildiği için uyarıldı\n\`\`\`${msg.content}\`\`\``
              )
        ...
        if (links && links.length > 0) {
          ...
          if (userLimit.count >= 5) {
            ...
            msg.channel.send({content:`${member} Reklam yapmaya devam ettiğin için seni \` 5 Dakika \` susturdum`}).then(async (msg) => {
              setTimeout(async () => {
                if (msg && msg.deletable) await msg.delete();
              }, 9000);
            });
            ...
            logChannel.send({embeds:[ new EmbedBuilder()
              .setDescription(
                `<@${member.id}> Reklam bağlantısı gönderildiği için **5 Dakika** susturuldu`
              )
              .setFooter({ text: `Kullanıcı ID: ${member.id}` })
              .setTimestamp(),]})
          ...
          switch (m.customId) {
            ...
            case "linkkick": {
              ...
              const kickEmbed = new EmbedBuilder()
                .setTitle("Kicked")
                .setDescription(
                  `Reklam bağlantıları gönderdiğiniz için \`${msg.guild.name}\` sunucusundan atıldınız.`
                )
                .setTimestamp();
              ...
              member.send({
                ...
                member.kick({ reason: "Reklam Yaptığı İçin Kicklendi (Luhux was here kanzi)." });
              });
              break;
            }
          }
